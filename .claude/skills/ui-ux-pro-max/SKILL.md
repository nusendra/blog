...
The search script is vendored in this repo. Invoke it from the project root:

```bash
python .claude/skills/ui-ux-pro-max/scripts/search.py "<query>" --domain <domain>
```

If `python` is not found, try `python3`, then `/opt/homebrew/bin/python3.13`. Requires Python 3.x, no external dependencies.
